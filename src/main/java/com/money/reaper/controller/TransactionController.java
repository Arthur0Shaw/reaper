...
public class TransactionController {
    ...
    @PostMapping("/report")
    public ResponseEntity<List<Transaction>> getTransactionsDeatils(@RequestBody TransactionReportRequest txnReportrequest) {
        List<Transaction> transactions;

        if (txnReportrequest.getStartDate() != null && txnReportrequest.getStartDate() != null) {
            transactions = transactionService.getTransactionsByCreatedAt(txnReportrequest.getStartDate(), txnReportrequest.getStartDate());
        }
        else if (txnReportrequest.getDateIndexStart() != null && txnReportrequest.getDateIndexEnd() != null) {
            transactions = transactionService.getTransactionsByDateIndex(txnReportrequest.getDateIndexStart(), txnReportrequest.getDateIndexEnd());
        } 
        else {
